        ...
        public void FillBoardStorage()
        {
            ...
            for (int col = 1; col <= board.width; col++)
            {
                for (int row = 1; row <= board.height; row++)
                {
                    ...
                    GameObject essense = Instantiate(testText);
                    essense.GetComponent<RectTransform>().SetParent(testText.GetComponent<RectTransform>());
                    string text = "(" + col + "," + row + ")";
                    essense.GetComponent<Text>().text = text;
